...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pedido_id'], $_POST['nuevo_estado'])) {
    ...
    $estados_validos = ['en espera', 'en proceso', 'en envío', 'completado'];
...
foreach ($pedidos as $pedido) {
    if ($pedido['estado'] === 'completado') {
        $pedidos_completados[] = $pedido;
    } else {
        $pedidos_no_completados[] = $pedido;
    }
}
?>
        ...
        <?php else: ?>
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php foreach ($pedidos_no_completados as $pedido): ?>
                            <tr>
                                <td>
                                    <?= $pedido['id'] ?>
                                </td>
                                ...
                                <td>
                                    <form method="post" class="d-inline-flex align-items-center" style="gap:0.3rem;">
                                        ...
                                        <select name="nuevo_estado" class="form-select form-select-sm" required>
                                            <?php
                                            $estados = ['en espera', 'en proceso', 'en envío', 'completado'];
                                    ...
                                    <a href="index.php?view=detalle_admin&id=<?= $pedido['id'] ?>"
